...
# 配置日志记录器：同时输出到文件和控制台
log_filename = "training_log_1.txt"
...
class DLinearTiDE(nn.Module):
    ...
    def forward(self, x_load, x_cov):
        ...
        # --- Encoding ---
        # 展平输入
        seasonal_flat = seasonal.reshape(batch_size, -1) # [Batch, Seq_Len]
        cov_flat = x_cov.reshape(batch_size, -1)         # [Batch, (Seq+Pred)*n_cov]
        
        # 拼接进入Encoder
        encoder_input = torch.cat([seasonal_flat, cov_flat], dim=1)
        encoded = self.encoder(encoder_input) # [Batch, Hidden]
        ...
        pred_trend = self.trend_linear(trend_flat) # [Batch, Pred_Len]
        pred_trend = pred_trend.unsqueeze(-1)
        
        # 最终结果
        pred_load = pred_seasonal + pred_trend
...
if __name__ == "__main__":
    ...
    logger.info("损失曲线已保存为 loss_curve_1.png")
    ...
    plt.ylabel('Load')
    plt.legend()
    plt.savefig(f'forecast_sample_{sample_idx}.png')
    logger.info(f"可视化结果已保存为 forecast_sample_{sample_idx}.png")
